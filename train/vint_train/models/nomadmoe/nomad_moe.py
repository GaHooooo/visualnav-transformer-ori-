...
from vint_train.models.nomadmoe.MoeTransformer import SoftMoEEncoder, SoftMoEEncoderLayer

class NoMaD_MOE(nn.Module):
    def __init__(
        self,
        context_size: int = 5,
        num_experts: int = 8,
        ...
        self.encoder_layer = SoftMoEEncoderLayer(
        ...
        self.encoder = SoftMoEEncoder(self.encoder_layer, num_layers=mha_num_attention_layers)
    ...
    def forward(self, obs_img: torch.tensor, goal_img: torch.tensor, input_goal_mask: torch.tensor = None) -> Tuple[
        ...
        obsgoal_img = torch.cat([obs_img[:, 3 * self.context_size:, :, :], goal_img],
                                dim=1)  # concatenate the obs image/context and goal image --> non image goal?
        ...
        obs_encoding_tokens = self.encoder(obs_encoding, src_key_padding_mask=src_key_padding_mask)
